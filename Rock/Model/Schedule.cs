...
namespace Rock.Model
{
    ...
    public partial class Schedule : Model<Schedule>, ICategorized, IHasActiveFlag
    {
        ...
        public virtual ScheduleType ScheduleType
        {
            get
            {
                var calendarEvent = GetCalendarEventV2();
        ...
        internal bool EnsureEffectiveStartEndDates()
        {
            ...
            var calEvent = GetCalendarEventV2();
            if ( calEvent == null )
            {
                return false;
            }
            ...
            else if ( countRules.Any() )
            {
                ...
                var occurrences = GetOccurrencesV2( DateTime.MinValue, DateTime.MaxValue );
        ...
        /// </value>
        [RockObsolete( "1.9" )]
        [Obsolete( "Use GetCalendarEventV2() instead " )]
        ...
        public virtual Ical.Net.Event GetCalendarEventV2()
        {
            return ScheduleICalHelperV2.GetCalendarEvent( iCalendarContent );
        }
        ...
        /// <returns></returns>
        [RockObsolete( "1.12" )]
        [Obsolete( "Use GetOccurrencesV2() instead." )]
        ...
        /// <param name="endDateTime">The end date time.</param>
        /// <returns></returns>
        public IList<Occurrence> GetOccurrencesV2( DateTime beginDateTime, DateTime? endDateTime = null )
        {
            return this.GetOccurrencesV2( beginDateTime, endDateTime, null );
        }
        ...
        /// <param name="scheduleStartDateTimeOverride">The schedule start date time override.</param>
        /// <returns></returns>
        public IList<Occurrence> GetOccurrencesV2( DateTime beginDateTime, DateTime? endDateTime, DateTime? scheduleStartDateTimeOverride )
        {
            ...
            if ( calEvent.DtStart != null )
            {
                ...
                foreach ( var occurrence in endDateTime.HasValue ?
                    ScheduleICalHelperV2.GetOccurrences( calEvent, beginDateTime, endDateTime.Value ) :
                    ScheduleICalHelperV2.GetOccurrences( calEvent, beginDateTime ) )
                {
                    bool exclude = false;
                    if ( exclusionDates.Any() && occurrence.Period.StartTime != null )
                    {
        ...
        public virtual List<CheckInTimes> GetCheckInTimes( DateTime beginDateTime )
        {
            ...
            if ( IsCheckInEnabled )
            {
                var occurrences = GetOccurrencesV2( beginDateTime, beginDateTime.Date.AddDays( 1 ) );
        ...
        public virtual List<DateTime> GetScheduledStartTimes( DateTime beginDateTime, DateTime endDateTime )
        {
            ...
            var occurrences = GetOccurrencesV2( beginDateTime, endDateTime );
        ...
        {
            var calEvent = this.GetCalendarEventV2();
            if ( calEvent != null && calEvent.DtStart != null )
            {
                ...
                var occurrences = GetOccurrencesV2( time.Date );
    ...
    /// <summary>
    /// A helper class for processing ICal schedules.
    /// </summary>
    /// <remarks>
    /// This class uses the iCal.Net implementation of the iCalendar (RFC 5545) standard.
    /// </remarks>
    public static class ScheduleICalHelperV2
    {
        private static object _initLock;
        private static Dictionary<string, Ical.Net.Event> _iCalSchedulesEx = new Dictionary<string, Ical.Net.Event>();

        static ScheduleICalHelperV2()
        {
            // iCal.Net.LoadFromStream is not threadsafe, so use locking
            ScheduleICalHelperV2._initLock = new object();
        }
        ...
        public static Ical.Net.Event GetCalendarEvent( string iCalendarContent )
        {
            ...
            lock ( ScheduleICalHelperV2._initLock )
            {
                if ( _iCalSchedulesEx.ContainsKey( trimmedContent ) )
                {
                    return _iCalSchedulesEx[trimmedContent];
                }
                ...
                if ( calendarList.Count > 0 )
                {
                    ...
                    if ( calendar != null )
                    {
                        ...
                        _iCalSchedulesEx.AddOrReplace( trimmedContent, calendarEvent );
                    }
                }
            }
    ...
    /// </summary>
    [RockObsolete( "v1.12" )]
    [Obsolete( "Use ScheduleICalHelperV2 instead." )]
